const MyNotify = (message, type = 'info', timeout = 3000, position = 'top-right') =>
{
  ...
  if (!document.querySelector('#my-notify'))
  {
    ...
    let importFonts = "";
    if (
      !document.querySelector('link[href*="fonts.googleapis.com/css2?family=Roboto"]') ||
      !document.querySelector('link[href*="fonts.googleapis.com/icon?family=Material+Icons"]')
    )
    {
      importFonts = `
  ...
  notification.classList.add('my-custom-notify', `my-notify-${type}`);
  notification.innerHTML = `
        <span class="my-icons">${icons[type] || 'info'}</span>
  ...
  setTimeout(() =>
  {
    notification.classList.add('my-hide');
